...
struct WorkspaceContentView: View {
    ...
    private func createNewTerminal(for task: WorkTask? = nil, worktreeBranch: String? = nil, provider: AIProvider = .claude, gridName: String? = nil, isolate: Bool = false, reviewPostCompletion: Bool = false) {
        ...
        // Add isolation flags
        if isolate {
            command += " --auto-worktree --allow-dirty"
        }
    ...
    var body: some View {
        mainContent
            ...
            .modifier(WorkspaceSheetModifier(
                ...
                onCreateTerminal: { task, branch, provider, gridName, isolate in
                    createNewTerminal(for: task, worktreeBranch: branch, provider: provider, gridName: gridName, isolate: isolate, reviewPostCompletion: task != nil)
                }
            ))
...
private struct WorkspaceSheetModifier: ViewModifier {
    ...
    let onCreateTerminal: (WorkTask?, String?, AIProvider, String?, Bool) -> Void  // task?, branch?, provider, gridName?, isolate
        ...
        content
            ...
            .sheet(item: $agentPickerMode) { mode in
                AgentPickerPanel(
                    ...
                    workspacePath: workspace.path,
                    task: mode.task,
                    onCreateTerminal: { branchName, provider, gridName, isolate in
                        onCreateTerminal(mode.task, branchName, provider, gridName, isolate)
                    },
