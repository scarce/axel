...
///
/// Worktree behavior (toggled via Enter / Shift+Enter):
/// - Enter: launches on the selected worktree/main directly.
/// - Shift+Enter: auto-creates a tmp worktree (axel-tmp/*) for task isolation.
struct AgentPickerPanel: View {
    let workspaceId: UUID?
    let workspacePath: String?
    let task: WorkTask?
    let onCreateTerminal: (String?, AIProvider, String?, Bool) -> Void  // branchName?, provider, gridName?, isolate
    ...
    var body: some View {
        ...
        .onKeyPress(keys: [.return], phases: .down) { keyPress in
            let autoWorktree = keyPress.modifiers.contains(.shift)
            if !isWorktreeFieldFocused || !newWorktreeBranch.isEmpty {
                confirmSelection(autoWorktree: autoWorktree)
                return .handled
            }
            return .ignored
        }
                        ...
                        TextField("branch name", text: $newWorktreeBranch)
                            ...
                            .onSubmit {
                                if !newWorktreeBranch.isEmpty {
                                    confirmSelection(autoWorktree: false)
                                }
                            }
                    } else {
    ...
    private func confirmSelection(autoWorktree: Bool = false) {
        // Determine branch based on selection and auto-worktree toggle:
        // - autoWorktree (Shift+Enter): create a nested tmp worktree for isolation
        // - normal (Enter): use the selected worktree/branch directly
        let branch: String? = {
            if autoWorktree {
                // Auto-worktree: create a tmp worktree from selected base
            ...
            // No auto-worktree: use the selected worktree directly
            if let item = selectedItem {
                switch item {
                case .mainWorktree:
                    return nil
        ...
        // New worktree requires non-empty branch name (only relevant without auto-worktree)
        if !autoWorktree, let item = selectedItem, case .newWorktree = item {
        ...
        switch rightItem {
        case .pane(let pane):
            onCreateTerminal(branch, pane.provider, nil, autoWorktree)

        case .grid(let grid):
            let provider = panes.first?.provider ?? .claude
            onCreateTerminal(branch, provider, grid.name, autoWorktree)
